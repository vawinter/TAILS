...
Here, we created minium convex polygons (MCP) and kernel density estimators (KDE) for
...
#Movement visualization for cats
```{r}
library(tidyverse)
library(lubridate)
library(move)
...
# create spatial frames with a OpenStreetMap 
...
```
#individual locations on an interactive leaflet map
```{r}
view_spatial(mC, path_colours = col, render_as = "leaflet")
```
#animate the movement paths into a GIF
```{r}
...
- interactive map
- change map extent
- cahnge tail color by group
- Next steps/what we werent able to do
